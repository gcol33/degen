...
from the data. When parameters are non-identifiable, the likelihood
function is flat in some direction, meaning infinitely many parameter
values produce the same likelihood.
...
Parameters may be theoretically identifiable but poorly determined in
practice:
...
- **Parameter classifications**: Each parameter is classified as
  identified, weakly identified, or non-identifiable

- **Condition number**: High values (\> 10^6) suggest ill-conditioning

- **Rank deficiency**: Indicates how many parameters are
  non-identifiable
...
The Fisher information matrix quantifies how much information the data
...
For the sum model, the null direction is (1, -1), meaning a and b can
trade off while keeping a + b constant.
...
![Bar chart showing eigenvalue spectrum of the Fisher information
matrix. One eigenvalue is positive indicating an identifiable direction,
while the other is near zero indicating a non-identifiable direction
(the null
space).](identifiability-diagnostics_files/figure-html/plot-eigenvalues-1.svg)
...
print(check)
#> <identifiability_result>
#> Overall: Model has POORLY IDENTIFIED parameters
#> 
#> Parameter status:
#>   alpha: NON-IDENTIFIABLE
#>   beta: NON-IDENTIFIABLE
#>   gamma: identified
#> 
#> Non-identified directions:
#>   0.71*alpha -0.71*beta
#> 
#> Condition number: 1.4e+03
#> Rank: 3 / 3
```
...
#> Condition number: 1402.097
cat("Eigenvalues:", round(info_eigenvalues(info), 4), "\n")
#> Eigenvalues: 112.4737 60.2475 0.0802
```

### Step 4: Profile likelihood (if needed)

When parameters appear weakly identifiable, profile likelihood provides
deeper insight:
...
1.  **Always check identifiability** before drawing conclusions from
    fitted parameters

2.  **Use multiple starting points** to detect if optimization finds
    different solutions
...
5.  **Report null directions** when non-identifiability is detected -
    they often have meaningful interpretation
